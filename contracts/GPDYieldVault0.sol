...
contract GPDYieldVault0 is ERC4626, Ownable {
    ...
    function _withdraw(address caller, address receiver, address owner, uint256 assets, uint256 shares) internal override {
        ...
        if (fee > 0) {
            IERC20(asset()).transfer(devAddress, fee);
        }

        IERC20(asset()).transfer(receiver, amountAfterFee);
    ...
    function depositTWOCATS(uint256 amount) external onlyOwner {
        ...
        twocats.transferFrom(msg.sender, address(this), amount);
    }

    function depositGERZA(uint256 amount) external onlyOwner {
        require(amount > 0, "Amount must be > 0");
        gerza.transferFrom(msg.sender, address(this), amount);
    }

    function depositPUSSY(uint256 amount) external onlyOwner {
        require(amount > 0, "Amount must be > 0");
        pussy.transferFrom(msg.sender, address(this), amount);
    }
